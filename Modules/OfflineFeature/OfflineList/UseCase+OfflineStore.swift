...
// from the offline document instead of the server. Pointing the existing screen at these is what
// makes it readable with no connection without a second implementation of it.
//
...
extension GetDocumentUseCase {
    ...
    // Safe to answer with the stored copy only because `SaveOfflineDocumentUseCase` fetches this
    // endpoint rather than keeping the list document it was handed: the list copy's `content` is
    // truncated, and serving that here would show a preview of the text as if it were all of it.
    static var offlineStore: Self {
        Self(execute: { id, server in
            @Shared(.offlineDocuments(server))
            var offlineDocuments: IdentifiedArrayOf<OfflineDocument>
...
extension GetDocumentsByIdsUseCase {
    ...
    // Whichever of the requested ids are themselves offline documents. A linked document that was
    // never saved offline cannot resolve there; showing the ones that are beats failing the whole
    // section.
    static var offlineStore: Self {
