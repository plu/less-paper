...
public struct DocumentType: Codable, Equatable, Hashable, Identifiable, Sendable {
    public typealias Id = Tagged<DocumentType, Int>
...
public extension DocumentType {
    ...
    static func testValue(
        ...
        owner: User.Id? = 3,
        slug: String = "test-document-type",
...
extension DocumentType: Comparable {
...
public extension DocumentType.Id {

    func get(_ server: Server) -> DocumentType? {
        @Dependency(\.apiCache)
        var apiCache

        return apiCache.documentType(id: self, server: server)
    }
}
