...
the places where an entity can still be created by someone without the permission to create one —
nine inline create controls in the end: five in the document form, four in the share sheet.
...
`canViewNotes` sits on the State behind each view that offers an entrance to the notes section —
`DocumentDetailReducer`, `DocumentRowReducer`, `DocumentViewerReducer` and `DocumentFormReducer` —
rather than on the notes reducer, because it is the view offering the entrance that decides whether
to render it at all; a gate held inside the section it is meant to hide could not hide it.
...
And **a permission the previous project already gated is honoured in one place and ignored in
others.** The document form and the share sheet both offer "create tag" inline. Project 2 hid the
tag list's create button for a user without `add_tag`; these buttons stayed. The share-sheet copy
was found during Project 2's review and recorded as inherited work. Following the same shape across
both sheets turned "create tag" into nine inline create controls — five in the document form
(tag, correspondent, document type, storage path, custom field) and four in the share sheet.

## Decisions

**A dead screen loses its entrance.** The Settings *Trash* row is gated on `delete_document`, and
without `view_note` every entrance to the notes section is dropped — all four of them, which is
what it takes for the section to be unreachable rather than merely harder to find. Project 2 hid
the six entity rows on exactly this reasoning; the difference is only that here the whole
destination goes, not a control inside it. A user is never walked into a screen where nothing
works, and no permission is ever reported as an error.
...
DocumentsFeature
  ...
  DocumentRowReducer.State       canEdit, canDelete, canViewNotes
  DocumentDetailReducer.State    canEdit, canViewNotes
  DocumentViewerReducer.State    canViewNotes
  ...
  DocumentFormReducer.State      canCreateTag, canCreateCorrespondent, canCreateDocumentType,
                                 canCreateStoragePath, canCreateCustomField, canViewNotes
TrashFeature
  TrashListReducer.State         canModifyTrash        [one property, not one per action: restore,
                                 delete forever and empty trash all read delete_document, so three
                                 properties would be three identical bodies]
ShareFeature
  ShareFormReducer.State         canCreateTag, canCreateCorrespondent, canCreateDocumentType,
                                 canCreateStoragePath, canImport
SettingsFeature
  SettingListReducer.State       canViewTrash, canImport, canScan
                                 [the rows Project 2 left ungated]
```
...
|---|---|---|
| Import | list toolbar, `SettingListView`, share sheet | `add_document` |
| Scan | list toolbar, `SettingListView` | `add_document` |
...
| Notes section entrance | row context menu, detail toolbar's viewer menu, viewer sheet's section menu, edit form's section menu | `view_note` |
...
section entrances are *entrances*, so each gets a test that the destination is absent, not merely
that a button inside it is. The trash row gets that test as a snapshot, because it is a row in a
rendered list. The notes entrances do not, and the reason is narrower than "the harness renders no
chrome": there are four of them, and two — the viewer sheet's section menu and the edit form's
section menu — sit in a rendered sheet body, not in nav-bar chrome, so that argument does not carry
them. What does carry all four is that every one is an item inside a `Menu`, whose content SwiftUI
does not build until the menu is opened, and this harness never opens one. So no image shows any of
them present or absent, and the reducer tests asserting `canViewNotes` on each of the four States
are the only assertions this gate gets. And selection mode gets a test for each half of its `or`:
`change_document` alone and `delete_document` alone must both show it, because an `and` would pass a
test that only ever grants both.
...
list's toolbar and the row context menu, and the unexpanded-`Menu` reason above covers all four
notes entrances, so none of those get a snapshot — a recorded reference for the detail toolbar's
notes entrance came back byte-identical to its ungated baseline, confirming it rather than being an
exception to it. The Settings
