...
[![Tuist dashboard](https://img.shields.io/badge/Tuist-dashboard-9E8CFF?logo=tuist&logoColor=white)](https://tuist.dev/plu/less-paper)
...
<https://apps.apple.com/en/app/less-paper/id6464425056>. It needs a paperless-ngx server offering
REST API version 8 or newer — the app negotiates the version on connect and turns an older server
...
- **Deep links** — open a document straight from a `lesspaper://` link
...
- **Support modules** — `Components` for shared UI and `DesignTokens` for the colour palette it draws from, `Logging` for the shareable error log, `TestSupport` / `ApiTestSupport` / `UITestSupport` / `SnapshotSupport` for the test helpers, and `MarketingKit`, which renders the App Store screenshots from committed captures.
