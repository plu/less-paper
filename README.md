...
```sh
mise run ci:test        # run the test suite
...
The project is generated by [Tuist](https://tuist.dev) from `Project.swift`, and split into 52 modules declared in `Tuist/ProjectDescriptionHelpers/Module.swift`:
...
- **App modules** — a few features still ship a tiny standalone app target (`CustomFieldsApp`, `DocumentsApp`, `ServersApp`) so they can be built and run in isolation, plus `ShareApp`, which hosts the share extension. These are being retired: UI tests used to drive one harness app per feature, and now drive the real app instead — see [Testing](#testing).
- **Support modules** — `Components` for shared UI, `TestSupport` / `ApiTestSupport` / `UITestSupport` for the test helpers.
...
236 test suites — 1160 tests — written with [Swift Testing](https://developer.apple.com/documentation/testing), covering unit tests, [snapshot tests](https://github.com/pointfreeco/swift-snapshot-testing) (233 recorded references) and XCUITest journeys that drive the real app.

The API layer is tested against a live paperless-ngx container rather than mocks, so the tests catch real API drift.

UI tests live in `AppUITests` and drive the assembled app end to end — onboarding, settings, and the create/edit/delete lifecycle of each entity — rather than a harness app per feature. Each test creates its own paperless user, so what it makes is invisible to every other test and the lists it opens start empty. Nine journeys run serially in a little over five minutes; they are the slow part of the suite, so CI reserves them for `main` and for pull requests labelled `UITests` or `TestFlight`.
