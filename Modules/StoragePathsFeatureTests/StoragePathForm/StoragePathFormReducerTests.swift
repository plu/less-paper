...
struct StoragePathFormReducerTests {

    @Test
    func test_permissionForm_permissionsUpdated() async throws {
        let store = TestStore(initialState: StoragePathFormReducer.State(
            server: .testValue()
        )) {
    ...
    func test_view_saveButtonTapped_success() async throws {
        let store = TestStore(initialState: StoragePathFormReducer.State(
            ...
            $0.saveStoragePath.execute = { _, _, _ in .testValue() }
        }
        ...
        await store.receive(\.delegate.storagePathSaved, .testValue())
    ...
    func test_view_saveButtonTapped_toastError() async throws {
        ...
        let store = TestStore(initialState: StoragePathFormReducer.State(
            ...
            $0.saveStoragePath.execute = { _, _, _ in throw ApiError.testValue() }
    ...
    func test_view_saveButtonTapped_fieldErrors() async throws {
        ...
        let store = TestStore(initialState: StoragePathFormReducer.State(
            ...
            $0.saveStoragePath.execute = { _, _, _ in
