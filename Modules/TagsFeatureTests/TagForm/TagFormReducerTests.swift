...
struct TagFormReducerTests {

    @Test
    func test_permissionForm_permissionsUpdated() async throws {
        let store = TestStore(initialState: TagFormReducer.State(
            server: .testValue()
        )) {
            TagFormReducer()
        }
    ...
    func test_view_saveButtonTapped_success() async throws {
        let store = TestStore(initialState: TagFormReducer.State(
            ...
            $0.saveTag.execute = { _, _, _ in .testValue() }
        }
        ...
        await store.receive(\.delegate.tagSaved, .testValue())
    ...
    func test_view_saveButtonTapped_toastError() async throws {
        ...
        let store = TestStore(initialState: TagFormReducer.State(
            ...
            $0.saveTag.execute = { _, _, _ in throw ApiError.testValue() }
